...
class GVFFrame(wx.Frame):
    def __init__(self, ac_id, ac_id_leader, desired_sigma):
        ...
        self.list_of_indexes = ['gvf_ke', 'gvf_kn', 'a', \
                'b']
    ...
    def formation(self):
        if self.XY[0] != -999 and self.XY_leader[0] != -999 and self.traj != None:
            ...
            if (self.a_index is not None) and (self.b_index is not None) :
                ...
                msga['ac_id'] = self.ac_id
                msga['index'] = self.a_index
                msga['value'] = u
                msgb = PprzMessage("ground", "DL_SETTING")
                msgb['ac_id'] = self.ac_id
                msgb['index'] = self.b_index
                msgb['value'] = u

                self.interface.send(msga)
                self.interface.send(msgb)
            
            #print "Sigma Mako: ", sigma_mako, " Sigma Jumper: ", sigma_jumper
            print "New radius: ", u, " Sigma error: ", e_sigma
