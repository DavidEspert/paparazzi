...
#define MAX_SIZE_MESSAGE 340// Taille maximale en byte des messages
...
void receiveEvent(int howMany){
  if (howMany>=1){
    if (Wire.receive()!=0){
      ...
      // Erreur du programme qui a lu un 0 en premier byte
      while (true){
...
void SBDsession(long n){
  ...
  // Si code erreur entre 0 et 4, c'est que c'est bon
    ...
    digitalWrite(Led,LOW);
    delay(500);
    for (int k=0;k<collation;k++){
      digitalWrite(Led,HIGH);
      delay(500);
      digitalWrite(Led,LOW);
      delay(500);
    }
    if (collation>4 && collation <16){ // Erreurs envoyees par la Gateway
      ...
      if (collation==16){
        smoothShutdown();
        delay(300000L); // On attend 5 min, parce que là, ça pue, donc ça sert à rien de surconsommer
